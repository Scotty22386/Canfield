/*********************************************************************
  Author:        Dana Vrajitoru, Scott Wickline IUSB, CS
...
void List::insertBack(int number)
{
	...
	if (head)                    //exact code used in copy function 
	{                            // just used with newNode. 
		tail->next = newNode;
	    tail = newNode;
	}
	else
...
bool List::isOrdered()
{
	ListIterator iter1, iter2; // Declares itterators to transverse array.
	
	iter1 = head; //Sets iterators = to head.
	iter2 = head;
	++iter2; //Adavances iter2 by 1 for comparison.
	
	while(iter2 != NULL)
	{

		if (*iter2 < *iter1) // compares adjacent elements for order
			return false;

		else
		{
			++iter1; //Moves foward through list until NULL
			++iter2;
		}
...
int List::countNegative()
{
	...
	{
		ListIterator iter1 = head;
		

		while (iter1 != NULL) //This loop traverses list looking for negatives. 
		{
			if (*iter1 < 0) // If element is less than zero increment count. 
				++count;

			++iter1;
		}

		return count; // Returns amount of negatives found. 
	}
...
int List::sum()
{
	...
	while (iter1 != NULL) 
	{
		temp = *iter1; //Sets value to int temp.
		total = temp + total; //Temp and total are added together. 
		++iter1;             // Increments to move through list.
	}

	return total; //Returns sum of all elements. 
...
void List::selectionSort()
{
	...
	/*This bit of code is a selections sort iter1 is compared to iter2 and are swapped if iter1
	is smaller than iter2. Each time iter2 moves throught the list iter1 increments 1 time until iter1 
	reaches a null pointer.
	*/
	while (iter1 != NULL)  // Main control loop.
	{
