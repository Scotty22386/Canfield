...
{   
    ...
    else if (from == 'T' && (to == 'f' || to == 'F'))
    {
        int temp;
        temp = score;
        tableau[fromId].concatenate(foundation[toId]);
        foundation[toId].sort();
        score = foundation[toId].stored();
        score = score - temp;
        checkscore(score);
...
void Canfield::checkscore(int score)
{
    char pick;

    if (score == 52)
    {
        cout << "Congrdulations you have won!!!! " << endl;
